...
- ""
- ""
date: '2020-10-19T21:28:43-05:00'
description: "Determining the expected price for a nights stay in Mexico's Capital"
draft: false
image: Mexico_AirBnB.jpeg
keywords: ""
slug: Airbnb2
...
In the following analysis we will look at AirBnB data from Mexico City. We will take a closer look at the data and try to build a predictive model that will help us determine the cost for two people to stay in Mexico City in an AirBnB  for four nights.

Our first step is to download information on all listings on AirBnB Mexico City from [https://insideairbnb.com]InsideAirbnb. 
...
In the following section we will take a deep look into the downloaded data. During this process we will clean the data as necessary and build improved dataframes step by step, which will then ultimately be used in our final regression model.
...
skim(listings)
```
The table above provides summary statistics for variables in the listings data frame and helps to improve our understanding of it. It will lay the groundwork for the analysis in the following section.
...
- Neighborhood cleansed: We find 16 neighborhoods in Mexico City, these could be summarised later in our analysis
...
- Square feet is missing many data points. This makes the variable essentially unusable. It will be removed in later steps.
...
- Bed, bedrooms and bathrooms have a small amount of missing data points. We can't safely assume these to just be 0 by nature so we will not alter them for now.
...
```
We find that the most popular minimum nights are: 1 night, 2 nights, 3 nights, 5 nights, 7 nights (descending order)
...
- Property Types
- Accommodates
- Super Hosts, Host Response & Acceptance Rate 
...
```{r hosts response}
# Investigate distribution of ost responses with histogram
listings5 %>% 
  ggplot() +
...
Due to the competitiveness and strict hosting policies of Airbnb, most hosts tend to have very high acceptance and response rates. Unusually low rates should be considered to be excpluded in the following section.


```{r superhost info}
# Calculate percentage of super hosts
listings5 %>% 
...
listings6 %>%
  ...
  # create boxplot for each property type
  group_by(prop_type_simplified) %>% 
  ggplot() +
  # creaze boxplot whilst ordering property types by median logged price
...
We saw a very skewed distribution of price. Outliers that charge for example over USD7,400 per night (or exp(8.91)) for a property do not seem to be legitimite properties and should therefore be excluded.

```{r exclude price outliers}
...
listings7 %>%
  ...
  labs(subtitle = "Histogram of Review Score Rating",
...
listings7 %>%
  ...
  # create linear model line
  geom_smooth(aes(x = review_scores_rating, y = price), method='lm') +
  labs(subtitle = "Review Score Rating vs Price",
       title = "Well rated properties seem to charge more on average \n - but driven by hugely expensive properties",
       x = "reviw score rating") +
  theme_economist()

```

Looking at review scores we saw that the ratings are left heavily skewed. This could likely be explained by the reciprocal rating mechanism of AirBnB, where hosts also rate the tenants and the tenants' score impacts their likelihood of them being accepted to a new booking. Irregularities at ratings of 20, 40, 60, and 80 may also be due to new listings with insufficient data, or AirBnB's rounding algorithms for lower values. Going forwards it could be considered to disregard low rated properties (e.g. <5) as these will have had substantial problems and might be outliers. A normal traveller wouldn't consider properties with such low ratings.
...
listings7 %>%
  ...
  labs(subtitle = "Cleaning Fee Histogram",
...
listings7 %>%
  ...
  labs(subtitle = "Cleaning Fee to Price Ratio Boxplot",
       title = "In relation to price some cleaning fees are non-sensical and should be excluded") +
  theme_economist()
...
knitr::include_graphics(here::here("combined_graphs.jpg"), error = FALSE)

 
```
The graphic above and our previous analysed have shown us a lot about our data.
...
Having analysed and cleaned our data, we would like to get a geographical feel for the remaining listings. This can be investigated below:
...
We will now build our predictive model for the cost for two people to stay in Mexico City.

For the target variable , we will use the cost for two people to stay at an Airbnb location for four nights.
...
We will once more use density plots to examine the distributions of price_4_nights and log(price_4_nights) and choose which one to use going forwards.
...
listings9 %>%
  ...
  labs(title = "Density plot of prices for 4 nights heavily right-skewed with no strict upper bound",
       subtitle = "Price 4 Nights Density Plot",
...
listings9 %>%
  ...
  labs(title = "Log 4-nights prices is closer to normal distribution",
       subtitle = "Logged Price 4 Nights Density Plot",
...
Looking at our Residuals vs Fitted as well as our Scale Location we can identify value clustering in both. We believe this to be driven by the dummy variables created for our inclusion of property type. Our linearity assumption and our equal variance assumption does not seem to be in breach however. Our normal Q-Q follows a straight line, therefore our normal distribution assumption is not broken.
...
Looking at our Residuals vs Fitted as well as our Scale Location again, we can identify even more value clustering in both. As before this is believed to be driven by the dummy variables created for our inclusion of property type and rooms. Our linearity assumption does not seem to be in breach. However  our equal variance assumption seems to not hold as well, we can see an increase in standardised residuals with increasing fitted values.  Our normal Q-Q follows a straight line, therefore our normal distribution assumption is not broken.
...
# Further Variables for Regression

Our dataset has many more variables, so here we are going to extend your analysis to further explore the data.

## Bathroom, bedrooms, beds and house size
...
Looking at our Residuals vs Fitted as well as our Scale Location we can see our linearity assumption and equal variance assumption seems to not hold. Our normal Q-Q mostly follows a straight line, therefore our normal distribution seems to mostly hold. Finally, we can find influential points that might skew our models result. This standalone model does not seem to perform well.
...
Here the Scale Location indicates an increasing variability our equal variance assumption seems to not hold as well. The remaining assumptions seem to hold reasonably well.
...
Again, the Scale Location indicates an increasing variability our equal variance assumption seems to not hold as well. The remaining assumptions seem to hold reasonably well, with some outliers in terms of constant leverage.
...
As above, the Scale Location indicates an increasing variability our equal variance assumption seems to not hold as well. The remaining assumptions seem to hold reasonably well, with some outliers in terms of constant leverage.
...
```
 ...
 We decided to separate neighbourhoods by attractiveness to tourists: touristy areas, less popular areas, and unpopular areas (rough areas or those far from centre), istead of simply by geographical or administrative divisions, as this expected to have a large impact on price.
...
Again the Scale Location indicates an increasing variability our equal variance assumption seems to not hold as well, driven by some outliers for high fitted values. The remaining assumptions seem to hold reasonably well, with some outliers in terms of constant leverage.
...
Our linearity assumption does not seem to be in breach. Similarly our equal variance assumption seems to not hold better again, with a small icnrease in variability driven by some outliers. 
...
Following our previous analysis we tried to build a model with similar predictive power but less variables.

```{r}
model9 <- lm(log(price_4_nights) ~   accommodates  + room_type   + review_scores_rating , 
             data = listings10)
...
# Summarise model estimates in dataframe
model9_estimates <- summary(model9)$coefficients[,1] 
model9_estimates <- exp(model9_estimates)
model9_estimates <- stack(model9_estimates)
model9_estimates <- model9_estimates %>% 
  select(ind, values)
...
Looking at our Residuals vs Fitted as well as our Scale Location our linearity  and equal variance assumption do not seem to be in breach. Our normal Q-Q follows a straight line, therefore our normal distribution assumption is not broken. We also dont seem to have any very influential points in terms of leverage


Expanding the maximum number of people accommodated by 1 is expected to increase 4-night prices by 9.6%. 

Moving from renting entire house/apartment to renting private rooms is expected to induce a price decrease of 71.13%. Moving from renting entire house/apartment to renting shared rooms is expected to induce a price decrease of 132.5%. However, this is an unrealistic assumption, as it means that we can expect to receive money if we were to rent shared rooms. Hence, we can only interpret from the data that renting a shared room will induce a percentage fall in 4-night prices even greater than that of renting private rooms. Finally, moving to a hotel room would lead to a 11.23% increase in price.

Lastly, ceteris paribus, an increase in review score ratings of 1 is expected to induce a 0.53% increase in 4-nights prices.
...
huxreg(
  ...
  list("Property type and review stats" = model1,
       ...
       "Variable efficient model" = model9),
...
Finally, we will use our best model four our price prediction. We will try to find Airbnb’s in Mexico City that are apartments with a private room, have at least 10 reviews, and an average rating of at least 90. Use your best model to predict the total cost to stay at this Airbnb for 4 nights. Include the appropriate 95% interval with your prediction. Report the point prediction and interval in terms of price_4_nights.

We will choose the following for our prediction:

- Apartment
- Private Room
- Exactly 10 reviews - every additional review is expected to increase costs
- Average rating of 90 - every additional rating point is expected to increase costs

Added assumption:
- 1 bed, 1 bathroom - being students we are happy to share a bed
- flexible cancellation policy - stricter policies tend to be for expensive proeprties
- accommodates 2 - we don't want to incur extra costs for extra people
- less popular neighborhood - we don't want to pay a premium 
- not superhost - we trust the locals who dont professionaly rent out their place
- location is not exact -  we can be spontaneous to avoid extra costs
...
Based on the model, we can expect the price of a 4-night accommodation in Mexico City to be USD1476, with the 95% confidence interval being a wide range from USD540 to USD4041.
